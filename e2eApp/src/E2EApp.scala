...
object E2EApp extends ZIOAppDefault:
  ...
  def liveRoutes(rootLayout: E2ERootLayout, assets: StaticAssets) =
    (Live.router @@ rootLayout)(
      ...
      (live / "healthy" / PathCodec.string("category")).params { (category, _, _) =>
        HealthyLiveView(category)
      },
      (live / "components").query[ComponentsLiveView.UrlParams]        -> ComponentsLiveView(),
      live / "js"                                                      -> JsLiveView(),
      live / "colocated"                                               -> ColocatedLiveView(),
      (live / "upload").queryOptional[String]("auto_upload")           -> UploadLiveView(),
      (live / "form").params(FormQueryParams.codec)                    -> FormLiveView(),
      (live / "form" / "nested").params(FormQueryParams.codec)         -> NestedFormLiveView(),
      live / "form" / "stream"                                         -> FormStreamLiveView(),
      (live / "form" / "dynamic-inputs").params(FormQueryParams.codec) ->
        FormDynamicInputsLiveView(),
      live / "form" / "feedback"                                -> FormFeedbackLiveView(),
      (live / "portal").query[PortalLiveView.QueryParams]       -> PortalLiveView(),
      (live / "errors").params(ErrorLiveView.QueryParams.codec) -> ErrorLiveView(),
      ...
      (live / "issues" / "3200" / PathCodec.string("tab")).params -> Issue3200LiveView(),
      ...
      (live / "issues" / "3529").queryOptional[String]("param") -> Issue3529LiveView(),
      (live / "issues" / "3530").queryOptional[String]("q")     -> Issue3530LiveView(),
      ...
      (live / "issues" / "3709").params.mapParams(_ => Option.empty[String]) ->
        Issue3709LiveView(),
      (live / "issues" / "3709" / PathCodec.int("id")).params
        .mapParams(id => Option(id.toString)) -> Issue3709LiveView(),
      ...
      (live / "issues" / "4094").queryOptional[String]("foo") ->
        Issue4094LiveView(),
