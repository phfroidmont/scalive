...
/** Defines the identity and optional retention policy of a LiveView stream.
  *
  * A stream is keyed by `name` within its owning LiveView or LiveComponent. Reuse the same
  * definition value, or a retention-adjusted value derived from it, throughout the lifecycle. The
  * runtime rejects a separately constructed definition for an existing stream name.
  *
  ...
  * The runtime rejects an empty name, an empty generated id, a throwing `id` or `domId`, an invalid
  ...
  * @tparam Id
  *   the stable domain identity type used to address items
  * @param name
  *   non-empty runtime key and the container id used by the `renderIn` helper
  * @param id
  *   stable function selecting an item's domain identity
  * @param domId
  *   stable function assigning each domain identity its row DOM id
  * @param limit
  *   optional retention limit applied by creation, reset, and insertion operations, defaulting to
  *   no limit
  */
final class LiveStreamDef[A, Id] private (
  val name: String,
  val id: A => Id,
  val domId: Id => String,
  val limit: Option[StreamLimit],
  private[scalive] val identity: LiveStreamDefIdentity):
  /** Returns a copy that retains at most the first `count` items.
    ...
    */
  def keepFirst(count: Int): LiveStreamDef[A, Id] =
    withLimit(Some(StreamLimit.KeepFirst(count)))

  /** Returns a copy that retains at most the last `count` items.
    *
    * Validation that `count` is positive is deferred until an operation applies the limit.
    */
  def keepLast(count: Int): LiveStreamDef[A, Id] =
    withLimit(Some(StreamLimit.KeepLast(count)))
  ...
  /** Returns a copy using `limit`; `None` disables retention pruning.
    ...
    */
  def withLimit(limit: Option[StreamLimit]): LiveStreamDef[A, Id] =
    LiveStreamDef(name, id, domId, limit, identity)

  /** Returns a copy with no retention limit. */
  def withoutLimit: LiveStreamDef[A, Id] =
    withLimit(None)
end LiveStreamDef

/** Convenience constructors for [[LiveStreamDef]]. */
object LiveStreamDef:
  /** Defines a stream using separate domain-ID selection and DOM-ID rendering functions. */
  def apply[A, Id](
    name: String,
    id: A => Id,
    domId: Id => String,
    ...
    LiveStreamDef(name, id, domId, limit, LiveStreamDefIdentity.fresh())

  private def apply[A, Id](
    name: String,
    id: A => Id,
    domId: Id => String,
    limit: Option[StreamLimit],
    identity: LiveStreamDefIdentity
  ): LiveStreamDef[A, Id] =
    new LiveStreamDef(name, id, domId, limit, identity)
  ...
  /** Defines DOM ids as `name-id(item)` using the selected id's string representation.
    ...
    */
  def byId[A, Id](name: String)(id: A => Id): LiveStreamDef[A, Id] =
    LiveStreamDef(name, id, value => s"$name-$value")

final private[scalive] class LiveStreamDefIdentity private ()

private[scalive] object LiveStreamDefIdentity:
  def fresh(): LiveStreamDefIdentity = new LiveStreamDefIdentity()
