...
| Security and session tokens | LiveView signing salt, session token, flash token, CSRF/connect params | Native coverage expanding | HMAC-signed tokens with max age, signed mount claims, signed flash tokens, root-layout key in session payload, shared ordinary-form and LiveSocket CSRF, automatic checked-POST fields, and stale joins for invalid websocket CSRF. | Token format still has TODOs around salt/messagepack. Claims are signed but not encrypted. | High |
