...
| Nested LiveViews | `test/phoenix_live_view/integrations/nested_test.exs` | Partial | Nested LiveViews render during disconnected HTTP mount, and connected nested LiveViews can be registered, joined, handle isolated events, clean up on parent/child leave, keep stable topics across parent re-renders/patches, defer render-removal cleanup until client confirmation, emit child-scoped navigation, and preserve sticky child sockets across parent leave. Remaining gaps are broader navigation/lifecycle edge cases. | High |
...
The core runtime is in place, so the highest-leverage follow-up work is now targeted parity around async lifecycle edge cases and broader nested LiveView navigation/lifecycle cases.
