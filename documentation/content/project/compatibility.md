...
| Endpoint/socket configuration (`:live_view` config, socket path, hibernation, long poll) | Partial | `Live.router.withSocketPath(PathCodec[Unit])` configures the socket path; validated `ZioHttpConfig` configures signing, maximum age, and secure cookies for `ZioHttp.routes`. | Decide which endpoint options matter on ZIO HTTP; hibernation and long poll are not implemented. |
...
| Test harness helpers (`Phoenix.LiveViewTest`) | Partial, intentional divergence | `scalive-testing` supports disconnected semantic HTML queries plus `ConnectedRender`/`ConnectedView` for production-admitted joins, typed messages, bindings, forms, nested views, and uploads. | Add explicit ordinary HTTP action following without cloning ConnTest or LiveViewTest; retain browser tests for real DOM patching and JavaScript behavior. |
