...
| Live sessions (`Phoenix.LiveView.Router.live_session/3`) | Native coverage substantial | `Live.session`, duplicate name validation, connected-navigation boundaries, claim-bearing `LiveSessionMountAspect`s, admission, and named live-session layouts are covered. | Session aspects are available only on named live-session builders; HTTP-only cookies and headers are consumed at session or document boundaries, and only minimal signed claims cross into connected mount. |
| Route-level `on_mount` (`Phoenix.LiveView.on_mount/1`) | Intentional divergence | Claimless `LiveRouteMountAspect`s receive typed path parameters and URL, compose in order, and run for disconnected HTTP, initial connected join, and navigation within a named live session. Their context is never serialized. `LiveRouteMountFailure` preserves redirect and 401/403/404 HTTP semantics while rejecting connected admission. | Route admission has no route-specific serialized claims. Invalid returns and contradictory continue/redirect states are unrepresentable. Mutable policy requires connected-turn guards and domain authorization. |
...
| `connected?/1` equivalent | Native parity covered | `LifecycleContext.connection` is `Connection.Disconnected` during HTTP rendering and `Connection.Connected(capabilities)` during connected lifecycle phases. | Connected-only capabilities are available only from the matched branch. |
...
| Connect params and connect info (`get_connect_params/1`, `get_connect_info/2`) | Partial | `ConnectedMetadata.connectParams` exposes untrusted JSON join parameters on connected capabilities. Session aspects receive a phase-specific request; route aspects receive typed path parameters and URL without original HTTP cookies or headers. The connected test harness owns and advances `_mounts`. | Scalive does not expose a typed connect-info API for peer, header, user-agent, or equivalent server-derived transport data. Phoenix-owned parameter keys, including `_mounts`, remain protocol metadata rather than a stable application contract. |
...
| Area and upstream reference | Status | Current evidence | Known gap or decision |
| --- | --- | --- | --- |
| Live navigation (`push_patch/2`, `push_navigate/2`, `redirect/2`, `<.link>`) | Native coverage substantial | Typed patch, navigate, replace, and redirect operations cover URL resolution, payloads, flash carryover, named live-session boundaries, history traversal, route-aspect reruns on navigation, and rejection of non-HTTP schemes and control-character bypasses in runtime string destinations. Destination route context is freshly derived for navigation; patches retain the mounted lifecycle. | Audit client/server fallback behavior across named live sessions and root-layout changes. |
...
| Forms and form events (form bindings, `Phoenix.Component.form/1`, `to_form/2`, recovery) | Native coverage substantial | Lossless payload parsing, typed `FormCodec`, change/submit bindings, used fields, definition-owned typed submitters, optional raw submitter metadata decoding, render helpers, component routing, dynamic-field recovery, and `phx-no-unused-field` are covered. | Decide which `to_form` and Ecto-specific behavior is out of scope. |
...
| Area and upstream reference | Status | Current evidence | Known gap or decision |
| --- | --- | --- | --- |
| Security and session tokens (signing, session, flash, CSRF, connect params) | Native coverage expanding | HMAC tokens with max age, signed session-mount claims and flash tokens, claimless route admission, typed redirects, hardened runtime string destinations, root-layout session data, shared form/socket CSRF, checked POST fields, and stale invalid-CSRF joins are covered. | Token salt and MessagePack details remain open. Session claims and flash values are signed, not encrypted; route-aspect output is never serialized. Unsafe mount redirects require an application-trusted destination. |
...
| Telemetry and observability (Phoenix telemetry and logger metadata) | Native coverage substantial | Public `LifecycleObserver` events cover disconnected rendering, joins, mounts, connected turns, handler and subscription failures, queue pressure, and termination. `LifecycleMetrics` maps them to a fixed low-cardinality ZIO Metrics contract; runtime and API tests cover ordering, redaction boundaries, correlation, labels, and sink-defect isolation. | The public contract is lifecycle-oriented rather than a copy of Phoenix telemetry names. Endpoint requests, access logs, infrastructure metrics, and a tracing adapter remain application concerns. |
| Test harness helpers (`Phoenix.LiveViewTest`) | Native coverage substantial, intentional divergence | `scalive-testing` supports disconnected semantic HTML queries, explicit ordinary GET/POST submission and 303 following, plus an in-process production transport client for signed routed joins, transport-owned `withAdmission` checks, typed messages, bindings, forms, nested views, hosted uploads, invalidation, reconnects, and live navigation. | Retain real-transport tests for WebSocket upgrade/framing and browser tests for DOM patching, reconnect timing, history, external uploads, and JavaScript behavior rather than cloning ConnTest or LiveViewTest. |
...
- Typed claim-bearing `LiveSessionMountAspect`s and claimless
  `LiveRouteMountAspect`s replace module-and-atom `on_mount` callbacks.
