...
| `Issue3814LiveView` | Previously bypassed normal submit binding handling with a raw event hook before `phx-trigger-action` submits. | Native triggered form submission includes the actual clicked submitter name/value without server compensation. | Uses a normal submit binding and relies on the client/native submitter path. | Fixed |
...
| `Issue3941LiveView` | Previously root state directly rendered or removed item divs. | Component-only patches inside a locked tree survive later stale locked-tree application across LiveComponents, nested headers, async assigns, and hook events. | Added real item LiveComponents with nested async header components and the hook/event path that locks the container. | Fixed |
| `Issue3953LiveView` | Previously faked a nested component with a root-rendered `data-phx-component` div. | Destroy messages for components inside a toggled nested child LiveView are scoped to the child, not the parent LiveView. | Verified the existing nested LiveView fixture with its own component against upstream. | Fixed |
...
| `Issue4066LiveView` | Previously root LiveView rendered the hooked input and halted the delayed event in a root raw-event hook. | A delayed hook `pushEventTo` targeted at a LiveComponent after that component/input disconnected is ignored without bubbling, crashing, or remounting the parent. | Verified the existing real LiveComponent fixture with component-targeted delayed hook events against upstream. | Fixed |
...
| `Issue4147LiveView` | Previously rendered the hook element inside the LiveView root. | A colocated hook outside the LiveView root mounts once and survives disconnect/reconnect behavior without duplicate mounting. | Rendered the hook outside `[data-phx-main]` through the E2E root layout for `/issues/4147`. | Fixed |
