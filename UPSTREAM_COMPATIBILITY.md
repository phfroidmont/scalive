...
| Lifecycle hooks | `test/phoenix_live_view/integrations/hooks_test.exs` | Partial native coverage | Scalive now has mount-registered typed root hooks for event, params, info, async, and after-render stages plus stateful component event/after-render hooks. Coverage includes attach order, halt/continue, event replies, duplicate-id rejection, detach, async task names, unsupported component stages, and after-render state persistence. Remaining gaps are route-level `onMount` and broader upstream error-shape assertions. | Medium |
...
The core component, nested LiveView, flash, async task, async assign, root hook, and component hook runtimes are covered, so the highest-leverage follow-up work is now route-level `onMount` and broader lifecycle error-shape parity.
