...
| Test harness helpers (`Phoenix.LiveViewTest`) | Native coverage substantial, intentional divergence | `scalive-testing` supports disconnected semantic HTML queries, explicit ordinary GET/POST submission and 303 following, plus an in-process production transport client for signed routed joins, physical admission, typed messages, bindings, forms, nested views, uploads, invalidation, reconnects, and live navigation. | Retain real-transport tests for WebSocket upgrade/framing and browser tests for DOM patching, reconnect timing, history, and JavaScript behavior rather than cloning ConnTest or LiveViewTest. |
