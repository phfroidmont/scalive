...
object DocumentationApplicationSpec extends ZIOSpecDefault:
  ...
  override def spec = suite("DocumentationApplicationSpec")(
    ...
    test("renders every generated page as disconnected HTML") {
      for
        ...
        _ <- ZIO.foreachDiscard(application.pages) { entry =>
               for
                 rendered <- DisconnectedRender.run(routes, Request.get(url(entry.page.route)))
                 document = Jsoup.parse(rendered.html, entry.page.route)
                 expectedTitle =
                   if entry.page.route == "/" then "Scalive"
                   else s"${entry.page.metadata.title} | Scalive"
                 expectedHeading = entry.page.source match
                 ...
                 missingOutlineIds = entry.page.outline.items
                                       .flatMap(flattenOutline)
                                       .map(_.id)
                                       .filter(id => document.getElementById(id) == null)
                 substantiveText = Option(document.selectFirst("main")).map { main =>
                                     val content = main.clone()
                                     content.select("h1, .docs-page-links").remove()
                                     content.text().trim
                                   }
                 failures = Vector(
                               Option.when(rendered.response.status != Status.Ok)("status"),
                               Option.when(document.title() != expectedTitle)("title"),
                               Option.when(
                                 document.select("meta[name=description]").attr("content") !=
                                   entry.page.metadata.description
                               )("description"),
                               Option.when(
                               ...
                               Option.when(
                                 Option(document.selectFirst("h1")).forall(_.text() != expectedHeading)
                               )("h1 title"),
                               Option.when(substantiveText.forall(_.isEmpty))("meaningful content"),
                               ...
                               Option.when(
                                 !document.select(".docs-page-links").text().contains(
                                     ...
                                     !document.select(".docs-page-links a").asScala.exists(link =>
                                       link.text() == "Edit this page" &&
                                         link.attr("href").contains(
                                           s"/${location.path}#L${location.line}"
                                         )
                                     )
                                   )("edit link")
                 ...
                 _ <-
                   ZIO
                     .fail(new AssertionError(s"${entry.page.route}: ${failures.mkString(", ")}"))
    ...
    test("renders editorial sections as flat indexes and preserves the API tree") {
      for
        ...
        learnLinks      =
          learnDocument.select(".docs-section-index > nav > ol > li > a").asScala.toVector
        guideLinks =
          guidesDocument.select(".docs-section-index > nav > ul > li > a").asScala.toVector
        guideGroups = guidesDocument.select(".docs-nav-group").asScala.toVector.map { heading =>
                        heading.text() -> heading
                          .nextElementSibling().select("a").asScala.toVector.map(_.attr("href"))
                      }
        projectLinks =
          projectDocument.select(".docs-section-index > nav > ul > li > a").asScala.toVector
      yield assertTrue(
        ...
        apiDocument
          .select(".docs-api-navigation nav > ul > li").asScala.toVector
    ...
    test("renders generated API summaries, signatures, and pinned sources") {
      for
        ...
        expectedSources =
          application.bundle.apiReference.symbols
    ...
    test("renders authored API directives as inline references with concise previews") {
      ...
      yield assertTrue(
        ...
        reference
          .select("code.docs-api-reference-signature, pre .docs-api-reference-signature").isEmpty,
        ...
        reference
          .select(".docs-api-reference-summary").text().contains(
    ...
    test("renders a bounded URL-filtered example catalog") {
      for
        ...
        renderedExampleIds = document
                               .select("[data-example-card]").asScala.toVector
      ...
      yield assertTrue(
        ...
        document
          .select("[data-standalone-lab] a[href='/examples/authentication/lab']").asScala
        ...
        document
          .select(
            "a[data-example-topic-filter][href='/examples?topic=keyed-rendering']"
          ).size() == 1,
    ...
    test("renders a representative source-backed example on its canonical detail route") {
      for
        application <- loadApplication
        assets      <- StaticAssets.load(StaticAssetConfig.classpath("public", assetNames))
        routes =
      ...
      yield assertTrue(
        ...
        counterExample
          .select(s"#$counterNestedId[data-phx-session][data-phx-parent-id]").size() == 1,
    ...
    test("serves tracked assets and leaves unknown paths as real 404 responses") {
      for
        ...
        routes = application
                   .routes(assets, security, config).provideEnvironment(
    ...
    test("serves a manifest-derived sitemap and self-contained robots policy") {
      ...
      yield assertTrue(
        ...
        locations == application.pages
          .map(_.page.route).distinct.sorted.map(config.publicOrigin.absolute),
