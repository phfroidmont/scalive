...
| Lifecycle hooks | `test/phoenix_live_view/integrations/hooks_test.exs` | Partial native coverage | Scalive now has mount-registered typed root hooks for event, params, info, and after-render stages, with attach order, halt/continue, event replies, duplicate-id rejection, detach, and after-render state persistence covered. Remaining gaps are route-level `onMount`, async hooks, component event/after-render hooks, and broader upstream error-shape assertions. | Medium |
...
The core component, nested LiveView, flash, async task, async assign, and root hook runtimes are covered, so the highest-leverage follow-up work is now targeted parity around async/component hooks and route-level `onMount`.
