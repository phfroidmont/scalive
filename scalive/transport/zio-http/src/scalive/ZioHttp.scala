...
object ZioHttp:
  ...
  final private case class ConnectedMountRejected(failure: LiveMountFailure)
      extends Exception(
        failure match
          case LiveMountFailure.Unauthorized(Some(reason)) =>
            s"connected mount rejected: $reason"
          case LiveMountFailure.Stale(Some(reason)) =>
            s"connected mount rejected: $reason"
          case _ => "connected mount rejected"
      )
  ...
  private def monotonicNanos: UIO[Long] = ZIO.succeed(java.lang.System.nanoTime())

  private[scalive] trait RuntimeObserverFactory:
    def connect(request: Request): RuntimeObserver

  private object RuntimeObserverFactory:
    val logging: RuntimeObserverFactory = new RuntimeObserverFactory:
      def connect(request: Request): RuntimeObserver = RuntimeObserver.logging
  ...
  def routes[R](application: LiveApplication[R], config: ZioHttpConfig): Routes[R, Nothing] =
    routes(application, config, LifecycleObserver.none)

  /** Assembles routes and reports structured lifecycle events to `observer`. */
  def routes[R](
    application: LiveApplication[R],
    config: ZioHttpConfig,
    observer: LifecycleObserver
  ): Routes[R, Nothing] =
    routes(application, config, observer, RuntimeObserverFactory.logging)

  private[scalive] def routes[R](
    application: LiveApplication[R],
    config: ZioHttpConfig,
    observerFactory: RuntimeObserverFactory
  ): Routes[R, Nothing] =
    routes(application, config, LifecycleObserver.none, observerFactory)
  ...
  private def routes[R](
    ...
    val getRoutes   = catalog.map(_.getRoute(config, lifecycleObserver))
    val socketRoute = websocketRoute(
      application.socketPath,
      catalog,
      config,
      lifecycleObserver,
      observerFactory
  ...
  /** Assembles routes with shared security and structured lifecycle observation. */
  def routes[R](
    application: LiveApplication[R],
    security: LiveSecurity,
    observer: LifecycleObserver
  ): Routes[R, Nothing] =
    routes(application, security.config, observer)
  ...
  private[scalive] def routes[R](
    ...
    routes(application, security.config, LifecycleObserver.none, observerFactory)
  ...
  final private[scalive] case class SessionMountClaims(values: Vector[String])
  ...
  private[scalive] def transferAdmissionOwnership(
    admission: Option[PreparedAdmission],
    pending: Ref[Option[PreparedAdmission]],
    active: Ref[Option[PreparedAdmission]]
  ): UIO[Unit] =
    (ZIO.foreachDiscard(admission)(_.commit) *>
      active.set(admission) *>
      pending.set(None)).uninterruptible
  ...
  sealed private[scalive] trait CompiledRoute[-R]:
    ...
    final def getRoute(
      ...
      RoutePattern(Method.GET, pathCodec) -> handler { (path: PathParams, request: Request) =>
        disconnected(path, request, config, observer).catchAllCause { _ =>
    ...
    protected def disconnectedLifecycle(
      ...
      R,
      Response,
      (RootLifecycle[Msg, Model], Any, SessionMountClaims, LiveRootLayout[PathParams, Any])
    ]
    ...
    private def renderDisconnected(
      ...
      lifecycle: RootLifecycle[Msg, Model],
      erasedContext: Any,
      mountClaims: SessionMountClaims,
      ...
      for
        ...
        renderStarted <- monotonicNanos
        failureStage  <- Ref.make(LifecycleFailureStage.Mount)
        outcome       <- (for
                     mountStarted <- monotonicNanos
                     mountExit    <- lifecycle.mount(turn.mountContext).exit
                     mountEnded   <- monotonicNanos
                     mount = LifecycleMount.Disconnected(turn.lifecycle.value)
                     _ <- mountExit match
                            case Exit.Success(_) =>
                              observer.safely(
                            ...
                            case Exit.Failure(cause) =>
                              observer.safely(
                                LifecycleEvent.MountFailed(
                                  mount,
                                  math.max(0L, mountEnded - mountStarted),
                                  lifecycleError(LifecycleFailureStage.Mount, cause)
                                )
                              )
                     mounted         <- ZIO.suspendSucceed(mountExit)
                     _               <- failureStage.set(LifecycleFailureStage.Handler)
                     ...
                     response        <- mountNavigation match
                                   ...
                                   case None =>
                                     ...
                                     yield response
                   yield response).exit
        renderEnded <- monotonicNanos
        duration = math.max(0L, renderEnded - renderStarted)
        failedAt <- failureStage.get
        _        <- outcome match
               case Exit.Success(_) =>
                 observer.safely(
                   LifecycleEvent.DisconnectedRenderSucceeded(turn.lifecycle.value, duration)
                 )
               case Exit.Failure(cause) =>
                 observer.safely(
                   LifecycleEvent.DisconnectedRenderFailed(
                     ...
                     duration,
                     lifecycleError(failedAt, cause)
                   )
                 )
        response <- ZIO.suspendSucceed(outcome)
      yield response
    ...
    private def base[R, A, In, Ctx, Message, State](
      ...
      protected def connectedLifecycle(
        ...
        runConnectedContext(
          ...
          request,
          ().asInstanceOf[In],
          SessionMountClaims(claims.sessionMountClaims),
          admission
          ...
          (ZIO
            ...
            }).onError(_ => ZIO.foreachDiscard(connected.admission)(_.rollback))
        }
  ...
  private def runDisconnectedSessionPipeline[R, In, Ctx](
    pipeline: LiveSessionMountPipeline[R, In, Ctx],
    mountRequest: LiveSessionMountRequest,
    input: In
  ): ZIO[R, Response, (Ctx, SessionMountClaims)] = pipeline match
    case _: LiveSessionMountPipeline.Identity[In] =>
      ZIO.succeed(input -> SessionMountClaims(Vector.empty))
    case thenNode: LiveSessionMountPipeline.Then[
          r,
          r1,
          In,
          previous,
      ...
      runDisconnectedSessionPipeline(thenNode.previous, mountRequest, input).flatMap {
        case (previousContext, previousClaims) =>
          thenNode.aspect.disconnected(mountRequest, previousContext).map { case (claim, output) =>
            ...
            thenNode.append.append(previousContext, output) ->
              SessionMountClaims(previousClaims.values :+ encoded)
          }
      }
    case controlled: LiveSessionMountPipeline.Controlled[
          r,
          r1,
          In,
          previous,
      ...
      runDisconnectedSessionPipeline(controlled.previous, mountRequest, input).flatMap {
        case (previousContext, previousClaims) =>
          controlled.aspect.disconnected(mountRequest, previousContext).map {
            case (claim, output) =>
              ...
              controlled.append.append(previousContext, output) ->
                SessionMountClaims(previousClaims.values :+ encoded)
          }
      }

  private def runRoutePipeline[R, A, In, Ctx](
    pipeline: LiveRouteMountPipeline[R, A, In, Ctx],
    mountRequest: LiveRouteMountRequest[A],
    input: In
  ): ZIO[R, LiveRouteMountFailure, Ctx] = pipeline match
    case _: LiveRouteMountPipeline.Identity[A, In] => ZIO.succeed(input)
    case thenNode: LiveRouteMountPipeline.Then[r, r1, A, In, previous, out, Ctx] =>
      runRoutePipeline(thenNode.previous, mountRequest, input).flatMap { previous =>
        thenNode.aspect
          .run(mountRequest, previous).map(output => thenNode.append.append(previous, output))
      }

  private def runConnectedSessionPipeline[R, In, Ctx](
    pipeline: LiveSessionMountPipeline[R, In, Ctx],
    mountRequest: LiveSessionMountRequest,
    input: In,
  ...
  private def runDisconnectedContext[R, A, In, Ctx](
    ...
    request: Request,
    input: In
  ): ZIO[R, Response, (Ctx, SessionMountClaims)] = context match
    case _: LiveRouteContext.Direct[A] => ZIO.succeed(((), SessionMountClaims(Vector.empty)))
    case environment: LiveRouteContext.Environment[r, A] =>
      ZIO
        .environmentWith[r](_.get(using environment.tag)).map(
          _ -> SessionMountClaims(Vector.empty)
        )
    ...
    case mounted: LiveRouteContext.Mounted[r, A, In, Ctx] =>
      runRoutePipeline(mounted.pipeline, LiveRouteMountRequest(path, request.url), input)
        .mapError(_.disconnectedResponse).map(_ -> SessionMountClaims(Vector.empty))
    case session: LiveRouteContext.SessionMounted[rs, rr, A, sessionCtx, routeIn, routeCtx] =>
      runDisconnectedSessionPipeline(
        session.session,
        LiveSessionMountRequest(request),
        ()
        ...
        ).map { case (routeContext, _) =>
          (sessionContext -> routeContext) -> sessionClaims
        }
      }
      ...
      runConnectedContext(environment.previous, path, request, input, claims, transport).flatMap {
        previous =>
          ZIO
            ...
            ).onError(_ => ZIO.foreachDiscard(previous.admission)(_.rollback))
      }
    case mounted: LiveRouteContext.Mounted[r, A, In, Ctx] =>
      for
        _      <- rejectExtraClaims(claims)
        result <- runRoutePipeline(
                    mounted.pipeline,
                    LiveRouteMountRequest(path, request.url),
                    input
                  ).mapError(_.connectedFailure)
      yield ConnectedContext(result, None)
    case session: LiveRouteContext.SessionMounted[rs, rr, A, sessionCtx, routeIn, routeCtx] =>
      for
        sessionResult <- runConnectedSessionPipeline(
                           session.session,
                           LiveSessionMountRequest(request),
                           (),
                           claims.values,
        ...
        routeContext <- runConnectedContext(
                          ...
                          SessionMountClaims(Vector.empty),
                          transport
                        ).onError(_ => ZIO.foreachDiscard(sessionResult.admission)(_.rollback))
        _ <- ZIO
  ...
  private def rejectExtraClaims(claims: SessionMountClaims): IO[LiveMountFailure, Unit] =
    ZIO
      .fail(LiveMountFailure.unauthorized("unexpected mount claims"))
      .when(claims.values.nonEmpty).unit
  ...
  private def websocketRoute[R](
    ...
    pattern -> handler { (request: Request) =>
      ...
      if !originAllowed then ZIO.succeed(Response.forbidden)
        ...
        val observer = observerFactory.connect(request).withLifecycleObserver(lifecycleObserver)
  ...
  final private case class JoinedUpload(
    ...
    progressEvent: Ref[Option[PhoenixUploadProgress]])
  ...
  final private class TransportTermination private (
    ...
    def routeOrFail[R](effect: => ZIO[R, Throwable, Unit]): ZIO[R, Throwable, Unit] =
      disconnecting.modify(current => !current -> current).flatMap {
        case true  => effect
        case false => ZIO.fail(InProcessTransportClosed())
      }
  ...
  final private[scalive] class InProcessTransportClosed private[ZioHttp] ()
      extends Exception("in-process LiveView transport is closed")

  private enum InProcessInput:
    case Envelope(value: PhoenixEnvelope, completed: Promise[Throwable, Unit])
    case Binary(value: Chunk[Byte], completed: Promise[Throwable, Unit])
    case Message(
      topic: String,
      joinRef: PhoenixRef.Value,
      ref: PhoenixRef.Value,
      value: Any,
      completed: Promise[Throwable, Unit])

  final private[scalive] class InProcessTransport private[ZioHttp] (
    incoming: Queue[InProcessInput],
    outgoing: Queue[PhoenixEnvelope],
    runtime: SocketRuntime[?],
    closed: Promise[Nothing, Unit],
    failure: Ref[Option[Throwable]]):

    def send(envelope: PhoenixEnvelope): Task[Unit] =
      offer(completed => InProcessInput.Envelope(envelope, completed))

    def sendBinary(bytes: Chunk[Byte]): Task[Unit] =
      offer(completed => InProcessInput.Binary(bytes, completed))
    ...
    def sendMessage(
      ...
      offer(completed => InProcessInput.Message(topic, joinRef, ref, value, completed))

    def receive: Task[PhoenixEnvelope] =
      outgoing.poll.flatMap {
        case Some(envelope) => ZIO.succeed(envelope)
        case None           =>
          outgoing.take.raceFirst(
            closed.await *> outgoing.poll.flatMap {
              case Some(envelope) => ZIO.succeed(envelope)
              case None           => closedFailure
            }
          )
      }

    def html(topic: String, joinRef: PhoenixRef.Value): Task[String] =
      runtime.html(topic, joinRef)

    def currentUrl(topic: String, joinRef: PhoenixRef.Value): Task[URL] =
      runtime.currentUrl(topic, joinRef)

    def isJoined(topic: String, joinRef: PhoenixRef.Value): UIO[Boolean] =
      runtime.isJoined(topic, joinRef)

    def close: UIO[Unit] = runtime.close *> closed.await
    ...
    private def offer(make: Promise[Throwable, Unit] => InProcessInput): Task[Unit] =
      for
        ...
        accepted  <- incoming
                      .offer(make(completed))
                      .raceFirst(closed.await.as(false))
        _ <-
          if accepted then completed.await.raceFirst(closed.await *> closedFailure)
          else closedFailure
      yield ()

    private def closedFailure[A]: Task[A] =
      failure.get.flatMap {
        case Some(error) => ZIO.fail(error)
        case None        => ZIO.fail(InProcessTransportClosed())
      }
  end InProcessTransport

  final private class SocketRuntime[R](
    routes: Vector[CompiledRoute[R]],
    config: ZioHttpConfig,
    ...
    uploadFrame: Ref[Option[Chunk[Byte]]],
    ...
    def receiveEnvelope(envelope: PhoenixEnvelope): ZIO[R & Scope, Throwable, Unit] =
      termination.routeOrFail(handleEnvelope(envelope))

    def receiveText(text: String): ZIO[R & Scope, Throwable, Unit] =
      termination.route(
        ZIO
          .fromEither(PhoenixEnvelope.decode(text).left.map(Exception(_))).flatMap(handleEnvelope)
      )

    private def handleEnvelope(envelope: PhoenixEnvelope): ZIO[R & Scope, Throwable, Unit] =
      PhoenixProtocol.decodeEnvelope(envelope) match
        ...
        case Right(PhoenixInbound.Join(joinRef, ref, topic, payload)) =>
          ZIO.uninterruptibleMask { restore =>
            for
              joinStarted    <- monotonicNanos
              outcomeClaimed <- Ref.make(false)
              nestedClaims   <- ZioHttpSecurity.verifyNestedJoin(config, payload.session).either
              observedTarget <- Ref.make(
                                  nestedClaims.fold(
                                    _ => LifecycleJoinTarget.Root,
                                    _ => LifecycleJoinTarget.Nested
                                  )
                                )
              observedLifecycle <- Ref.make(nestedClaims.toOption.flatMap(_.childLifecycle))
              _                 <- restore(
                     joinLifecycle(
                       routes,
                       config,
                     ...
                     observedTarget.get.zip(observedLifecycle.get).flatMap {
                       case (target, lifecycle) =>
                         observeJoinRejected(
                           observer,
                           supervisor,
                           lifecycle,
                           target,
                           lifecycleJoinAttempt(payload),
                           joinStarted,
                           None,
                           outcomeClaimed,
                           LifecycleFailure.Interrupted
                         )
                     }
                   )
            yield ()
          }
        case Right(PhoenixInbound.UploadJoin(joinRef, ref, topic, entryRef, token)) =>
          joinUpload(
            config,
            joined,
            uploads,
        ...
        case Right(PhoenixInbound.UploadProgress(joinRef, ref, topic, payload)) =>
          offerUploadProgress(joined, uploads, writer, joinRef, ref, topic, payload)
        case Right(PhoenixInbound.Event(joinRef, ref, topic, payload)) =>
          offerEvent(joined, writer, joinRef, ref, topic, payload)
        case Right(PhoenixInbound.ComponentsWillDestroy(joinRef, ref, topic, cids)) =>
          offerComponentsWillDestroy(joined, writer, joinRef, ref, topic, cids)
        case Right(PhoenixInbound.ComponentsDestroyed(joinRef, ref, topic, cids)) =>
        ...
        case Right(PhoenixInbound.Leave(joinRef, ref, topic)) =>
          leaveLifecycle(supervisor, joined, writer, joinRef, ref, topic)
        case Right(PhoenixInbound.UploadLeave(joinRef, ref, topic, _)) =>
          leaveUpload(uploads, writer, joinRef, ref, topic)

    def receiveBinary(frame: WebSocketFrame): Task[Unit] = termination.route(
      collectUploadFrame(uploadFrame, frame)
        .flatMap(ZIO.foreachDiscard(_)(offerUploadChunk(uploads, writer, _)))
    )

    def receiveBinary(bytes: Chunk[Byte]): Task[Unit] =
      if bytes.length <= MaxFramePayloadBytes then
        termination.routeOrFail(offerUploadChunk(uploads, writer, bytes))
      else ZIO.fail(Exception("websocket message exceeds maximum payload"))
    ...
    def receiveMessage(
      ...
      ref: PhoenixRef.Value,
      value: Any
    ): Task[Unit] = termination.routeOrFail {
      joined.get.map(exactTopicGeneration(_, topic, joinRef, _.joinRef)).flatMap {
        case Some(entry) =>
          enqueueCorrelated(entry, joinRef -> ref)(command =>
            entry.connection.message(command, value)
          )
        case None => writer.offer(PhoenixOutput.error(joinRef, ref, topic, staleReason))
      }
    }

    def route[R1](effect: => ZIO[R1, Throwable, Unit]): ZIO[R1, Throwable, Unit] =
      termination.route(effect)

    def remoteClosed: UIO[Unit] = termination.remoteClosed

    def run(receive: ZIO[R & Scope, Throwable, Unit]): ZIO[R & Scope, Throwable, Unit] =
      receive
        .raceFirst(
    ...
    def html(topic: String, joinRef: PhoenixRef.Value): Task[String] =
      joined.get.map(exactTopicGeneration(_, topic, joinRef, _.joinRef)).flatMap {
        case None        => ZIO.fail(Exception(s"LiveView topic '$topic' is not joined."))
        case Some(entry) =>
          entry.projectionGate.withPermit {
            entry.renderedState.get.flatMap {
              case None => ZIO.fail(Exception(s"LiveView topic '$topic' has no rendered state."))
              case Some(state) =>
                ZIO.fromEither(
                  PhoenixRenderedEncoder.html(state).left.map(error => Exception(error.toString))
                )
            }
          }
      }
    ...
    def currentUrl(topic: String, joinRef: PhoenixRef.Value): Task[URL] =
      joined.get.map(exactTopicGeneration(_, topic, joinRef, _.joinRef)).flatMap {
        case None        => ZIO.fail(Exception(s"LiveView topic '$topic' is not joined."))
        case Some(entry) => entry.currentUrl.get
      }

    def isJoined(topic: String, joinRef: PhoenixRef.Value): UIO[Boolean] =
      joined.get.map(exactTopicGeneration(_, topic, joinRef, _.joinRef).nonEmpty)

    def close: UIO[Unit] = termination.disconnect

    private def cleanup: UIO[Unit] = RuntimeCleanup.all(
      Vector(
  ...
  private def makeSocketRuntime[R](
    routes: Vector[CompiledRoute[R]],
    config: ZioHttpConfig,
    socketRequest: Request,
    csrfCookie: Option[String],
    csrfToken: Option[String],
    observer: RuntimeObserver,
    sendEnvelope: (PhoenixEnvelope, String) => Task[Unit],
    closeSocket: UIO[Unit]
  ): ZIO[Scope, SerialWriter.Error, SocketRuntime[R]] =
    for
      writer <- SerialWriter.make[PhoenixEnvelope](PhysicalWriterSize) { envelope =>
                  val encoded = PhoenixEnvelope.encode(envelope)
                  val send    = sendEnvelope(envelope, encoded)
                  observer.takeOutput(envelope) match
                    case None        => send
                    case Some(trace) =>
                      send.foldCauseZIO(
                        cause => observer.frameFailed(trace) *> ZIO.failCause(cause),
                        _ =>
                          observer.frame(
                            trace,
                            traceProtocol(envelope),
  ...
  private[scalive] def inProcessTransport[R](
    application: LiveApplication[R],
    config: ZioHttpConfig,
    socketRequest: Request,
    csrfCookie: Option[String],
    csrfToken: Option[String]
  ): ZIO[R & Scope, Throwable, InProcessTransport] =
    for
      routes   <- ZIO.attempt(validate(application))
      incoming <- Queue.bounded[InProcessInput](PhysicalWriterSize)
      outgoing <- Queue.bounded[PhoenixEnvelope](PhysicalWriterSize)
      closed   <- Promise.make[Nothing, Unit]
      failure  <- Ref.make(Option.empty[Throwable])
      observer = RuntimeObserver.logging
      runtime <- makeSocketRuntime(
                   routes,
                   config,
                   socketRequest,
                   csrfCookie,
                   csrfToken,
                   observer,
                   (envelope, _) => outgoing.offer(envelope).unit,
                   ZIO.unit
                 )
      receive = incoming.take.flatMap {
                  case InProcessInput.Envelope(value, completed) =>
                    runtime.receiveEnvelope(value).exit.flatMap { exit =>
                      completed.done(exit).unit *> ZIO.suspendSucceed(exit)
                    }
                  case InProcessInput.Binary(value, completed) =>
                    runtime.receiveBinary(value).exit.flatMap { exit =>
                      completed.done(exit).unit *> ZIO.suspendSucceed(exit)
                    }
                  case InProcessInput.Message(topic, joinRef, ref, value, completed) =>
                    runtime.receiveMessage(topic, joinRef, ref, value).exit.flatMap { exit =>
                      completed.done(exit).unit *> ZIO.suspendSucceed(exit)
                    }
                }.forever
      _ <- runtime
             .run(receive)
             .onError(cause => failure.set(Some(cause.squash)))
             .ensuring(closed.succeed(()).unit)
             .forkScoped
      _ <- ZIO.addFinalizer(runtime.close *> closed.await *> incoming.shutdown *> outgoing.shutdown)
    yield InProcessTransport(incoming, outgoing, runtime, closed, failure)
    ...
    for
      ...
      receive = channel.receiveAll {
                  case ChannelEvent.Read(WebSocketFrame.Text(text))
                    ...
                    runtime.receiveText(text)
                  case ChannelEvent.Read(frame: WebSocketFrame.Binary) =>
                    runtime.receiveBinary(frame)
                  ...
                  case ChannelEvent.Read(_: WebSocketFrame.Close) => runtime.remoteClosed
                  case ChannelEvent.Unregistered                  => runtime.remoteClosed
  ...
  private[scalive] def lifecycleJoinAttempt(join: RootJoin): LifecycleJoinAttempt =
    def counter(name: String): Option[Long] =
      join.params.get(name).flatMap {
        case Json.Num(value) => scala.util.Try(value.longValueExact()).toOption.filter(_ >= 0L)
        case _               => None
      }

    LifecycleJoinAttempt(counter("_mounts"), counter("_mount_attempts"))

  private def observeJoinSucceeded(
    observer: RuntimeObserver,
    supervisor: ConnectionSupervisor,
    connection: ConnectedLifecycle,
    ...
    started: Long,
    outcomeClaimed: Ref[Boolean]
  ): UIO[Unit] =
    ZIO.uninterruptible {
      outcomeClaimed.getAndSet(true).flatMap {
        case true  => ZIO.unit
        case false =>
          monotonicNanos.flatMap(ended =>
            observer.observeLifecycle(
              LifecycleEvent.JoinSucceeded(
                ConnectedLifecycleContext(
                  supervisor.id.value,
                  connection.lifecycle.value,
                  connection.epoch.value
                ),
                target,
                attempt,
                math.max(0L, ended - started)
              )
            )
          )
      }
  ...
  private def observeJoinRejected(
    observer: RuntimeObserver,
    supervisor: ConnectionSupervisor,
    ...
    outcomeClaimed: Ref[Boolean],
    failure: LifecycleFailure = LifecycleFailure.JoinRejected
  ): UIO[Unit] =
    ZIO.uninterruptible {
      outcomeClaimed.getAndSet(true).flatMap {
        case true  => ZIO.unit
          ...
          monotonicNanos.flatMap(ended =>
            observer.observeLifecycle(
              LifecycleEvent.JoinRejected(
                supervisor.id.value,
                lifecycle.map(_.value),
                target,
                attempt,
                math.max(0L, ended - started),
                LifecycleError(failure, cause)
              )
            )
          )
      }
  ...
  private def joinLifecycle[R](
    ...
    joinStarted: Long,
    outcomeClaimed: Ref[Boolean],
    observedTarget: Ref[LifecycleJoinTarget],
    observedLifecycle: Ref[Option[LifecycleId]],
    joinRef: PhoenixRef,
    ref: PhoenixRef,
    topic: String,
    join: RootJoin
  ): ZIO[R & Scope, Throwable, Unit] =
    val attempt = lifecycleJoinAttempt(join)
    effectiveJoinRef(joinRef, ref) match
      case None =>
        observedTarget.get.zip(observedLifecycle.get).flatMap { case (target, lifecycle) =>
          observeJoinRejected(
            observer,
            supervisor,
            lifecycle,
      ...
      case Some(effectiveRef) =>
        joinGate.withPermit {
          ZioHttpSecurity.verifyNestedJoin(config, join.session).either.flatMap {
            case Right(_) =>
              observedTarget.set(LifecycleJoinTarget.Nested) *>
                joinNested(
                  config,
                  csrfToken,
                  supervisor,
            ...
            case Left(_) =>
              registration.get.flatMap { registered =>
                ...
                admission.foldZIO(
                  ...
                  admitted =>
                    Ref.make(Option.empty[PreparedAdmission]).flatMap { pendingAdmission =>
                      val install = for
                        _ <- observedLifecycle.set(Some(LifecycleId(admitted.claims.lifecycle)))
                        ...
                        prepared <-
                          ZIO.uninterruptibleMask { restore =>
                            ...
                            ).flatMap(value => pendingAdmission.set(value.admission) as value)
                          }
                        ...
                        _ <- transferAdmissionOwnership(
                               prepared.admission,
                               pendingAdmission,
                               activeAdmission
                             )
                        _ <- observeJoinSucceeded(
                               observer,
                               supervisor,
                               connection,
                               LifecycleJoinTarget.Root,
                      ...
                      install
                        .onExit {
                          ...
                          case Exit.Failure(_) =>
                            pendingAdmission.get.flatMap(ZIO.foreachDiscard(_)(_.reject))
                        }
                        .catchAll { error =>
                          val clearInitial =
  ...
  private def joinNested(
    ...
    admitted.foldZIO(
      ...
      (claims, url, reservation) =>
        (for
          _               <- observedLifecycle.set(claims.childLifecycle)
          renderedState   <- Ref.make(Option.empty[PhoenixRenderedState])
          projectionGate  <- Semaphore.make(1L)
          eventGate       <- Semaphore.make(1L)
          correlations    <- Ref.make(Map.empty[CommandId, (PhoenixRef, PhoenixRef)])
  ...
  private def lifecycleSink(
    ...
    projectionGate
      .withPermit {
        ...
        def offer(envelope: PhoenixEnvelope): Task[Unit] =
          writer.offer(envelope)

        def offerFinal(envelope: PhoenixEnvelope): Task[Unit] =
          ZIO.suspendSucceed {
        ...
        output match
          case ConnectionOutput.Joined(delta, effects) =>
            update(delta)
              .map(addEffects(_, effects)).flatMap(json =>
                offerFinal(PhoenixOutput.join(joinRef, joinReplyRef, topic, json))
              )
          case ConnectionOutput.JoinedNavigation(delta, navigation, effects) =>
            if navigation.kind.isPatch then
              ...
              navigationJoinEnvelope(config, joinRef, joinReplyRef, topic, navigation)
                .map(addJoinEvents(_, effects)).flatMap(offerFinal)
          case ConnectionOutput.Reply(command, delta, effects) =>
            correlations.modify(current => current.get(command) -> (current - command)).flatMap {
              case Some((eventJoinRef, PhoenixRef.Null)) =>
                update(delta)
                  .map(addEffects(_, effects)).flatMap(json =>
                    offerFinal(PhoenixOutput.diff(eventJoinRef, topic, json))
                  )
              case Some((eventJoinRef, eventRef)) =>
                update(delta)
                  .map(addEffects(_, effects)).flatMap(json =>
                    offerFinal(PhoenixOutput.event(eventJoinRef, eventRef, topic, json))
                  )
                  ...
                  uploadReply match
                    case preflight: UploadPreflightView =>
                      encodeUploadPreflight(config, preflight).flatMap { response =>
                        val reply = PhoenixUploadProtocol.preflightReply(
                          eventJoinRef,
                          eventRef,
                          topic,
                          response,
                          Option.when(diff.fields.nonEmpty)(diff)
                        )
                        offerFinal(reply)
                      }
            ...
            correlations.modify(current => current.get(command) -> (current - command)).flatMap {
                ...
                update(delta)
                  .map(addEffects(_, effects)).flatMap { json =>
                    val diff = ZIO.when(json.fields.nonEmpty)(
                      offer(PhoenixOutput.diff(eventJoinRef, topic, json))
                    )
                    if navigation.kind.isPatch then
                      diff *> offerFinal(patchEnvelope(eventJoinRef, topic, navigation)) *>
              ...
              case Some((eventJoinRef, eventRef)) =>
                update(delta)
                  .map(addEffects(_, effects)).flatMap(json =>
                    if navigation.kind.isPatch then
                      offer(PhoenixOutput.event(eventJoinRef, eventRef, topic, json)) *>
                        offerFinal(patchEnvelope(eventJoinRef, topic, navigation)) *>
                        acknowledgePatch(navigation.destination)
                    else
          ...
          case ConnectionOutput.Rejected(command, _) =>
            correlations.modify(current => current.get(command) -> (current - command)).flatMap {
              ...
              case Some((eventJoinRef, eventRef)) =>
                offerFinal(PhoenixOutput.error(eventJoinRef, eventRef, topic, staleReason))
              case None => ZIO.fail(Exception(s"missing event correlation ${command.value}"))
            }
          case ConnectionOutput.Diff(delta, effects) =>
            update(delta)
              .map(addEffects(_, effects)).flatMap(json =>
                offerFinal(PhoenixOutput.diff(joinRef, topic, json))
              )
              ...
              .map(addEffects(_, effects)).flatMap(json =>
                val diff =
                  ZIO.when(json.fields.nonEmpty)(
                    offer(PhoenixOutput.diff(joinRef, topic, json))
                  )
                if navigation.kind.isPatch then
                  diff *> offerFinal(patchEnvelope(joinRef, topic, navigation)) *>
                    acknowledgePatch(navigation.destination)
                else
      ...
      }.tapError(_ => observer.failOutput(output))
  ...
  private def addJoinEvents(
    envelope: PhoenixEnvelope,
    effects: scalive.runtime.kernel.SessionEffects
  ): PhoenixEnvelope =
    if effects.clientEvents.isEmpty then envelope
    else
      val events = Json.Arr(
        effects.clientEvents.map(event => Json.Arr(Json.Str(event.name), event.payload))*
      )
      val payload = envelope.payload match
        case Json.Obj(fields) =>
          val values = fields.toMap
          values.get("response") match
            case Some(response: Json.Obj) =>
              Json.Obj(fields.map {
                case ("response", _) => "response" -> response.add("events", events)
                case field           => field
              }*)
            case _ => throw IllegalStateException("join navigation reply has no response object")
        case _ => throw IllegalStateException("join navigation reply payload is not an object")
    ...
    lifecycles.get.map(exactTopicGeneration(_, topic, joinRef, _.joinRef)).flatMap {
      case Some(joined) =>
        correlatedEventRefs(joined.joinRef, joinRef, ref) match
          case Some(eventRefs) =>
            joined.eventGate.withPermit {
              syncEventUploads(joined, event).flatMap { _ =>
                event.cid match
                  case None =>
                    ZIO.fromEither(event.toBindingPayload.left.map(Exception(_))).flatMap {
                      payload => enqueueLifecycleEvent(joined, eventRefs, event, payload)
                    }
                  case Some(cid) =>
          ...
          .withPermit {
            joined.renderedState.modify {
              case Some(state) =>
                ...
                val tokens             = cids.flatMap(cid =>
                  Option.when(deletedSet.contains(cid))(state.tokenForCid(cid)).flatten
                )
                (deleted, tokens) -> Some(updated)
              case None => (Vector.empty[Int], Vector.empty[Object]) -> None
            }
          }.flatMap { case (deleted, tokens) =>
            ZIO.when(tokens.nonEmpty)(joined.connection.destroyComponents(tokens)).unit *>
                    ...
                    joined.connection
                      .syncUploadProgress(
                        ...
                        case Left(_) =>
                          writer.offer(PhoenixOutput.error(joinRef, ref, topic, staleReason))
                        case Right(_) =>
                          rememberUploadProgress(uploads, joined, component, progress) *>
                            enqueueCorrelated(joined, eventRefs)(command =>
                              dispatchUploadProgressEvent(
                                joined,
                                component,
                                progress,
                                event,
  ...
  private def rememberUploadProgress(
    uploads: Ref[Map[String, JoinedUpload]],
    lifecycle: JoinedLifecycle,
    component: Option[ComponentInstanceId],
    progress: PhoenixUploadProgress
  ): UIO[Unit] =
    ZIO.foreachDiscard(progress.event) { _ =>
      uploads.get.flatMap(current =>
        ZIO.foreachDiscard(current.values) { upload =>
          val claims = upload.claims
          ZIO.when(
            (upload.lifecycle eq lifecycle) &&
              claims.componentInstanceId == component &&
              claims.uploadRef.value == progress.uploadRef &&
              claims.entryRef.value == progress.entryRef
          )(upload.progressEvent.set(Some(progress)))
        }
      )
    }
  ...
  private def offerUploadProgressEvent(
    ...
    enqueueUncorrelated(joined)(command =>
      dispatchUploadProgressEvent(joined, component, progress, event, command)
    )

  private def dispatchUploadProgressEvent(
    joined: JoinedLifecycle,
    component: Option[ComponentInstanceId],
    progress: PhoenixUploadProgress,
    event: String,
  ...
  end dispatchUploadProgressEvent

  private def offerFailedUploadProgress(upload: JoinedUpload): Task[Unit] =
    upload.progressEvent.get.flatMap(
      ZIO.foreachDiscard(_) { progress =>
        ZIO.foreachDiscard(progress.event)(event =>
          offerUploadProgressEvent(
            upload.lifecycle,
            upload.claims.componentInstanceId,
            progress,
            event
          ).catchSome {
            case ConnectionError.Closed           => ZIO.unit
            case _: ConnectionError.SessionFailed => ZIO.unit
          }
        )
      }
    )
                    ...
                    case Some(joined) =>
                      joined.connection
                        .admitUpload(
                          ...
                          case Right(worker) =>
                            Ref.make(Option.empty[PhoenixUploadProgress]).flatMap { progressEvent =>
                              val registration = JoinedUpload(
                                topic,
                                effective,
                                joined,
                                joined.connection,
              ...
              .uploadChunk(upload.worker, frame.payload).foldZIO(
                error =>
                  uploads.update(_ - frame.topic) *>
                    ...
                    offerFailedUploadProgress(upload),
                _ =>
                  writer.offer(
  ...
  private def enqueueLifecycleEvent(
    ...
    enqueueCorrelated(joined, eventRefs) { command =>
      joined.connection.submitBrowserEvent(
        command,
        BindingId.fromEncoded(event.event),
        payload,
        Some(event.toLiveEvent)
      )
    }.catchSome { case _: ConnectionError.SessionFailed => ZIO.unit }
      ...
      case Some(component) =>
        ZIO
          .fromEither(event.toBindingPayload.left.map(Exception(_))).flatMap { payload =>
            enqueueCorrelated(joined, eventRefs) { command =>
              joined.connection.submitComponentEvent(
                command,
                component,
                BindingId.fromEncoded(event.event),
                payload,
      ...
      command =>
        joined.correlations
          .modify { current =>
            ...
            case true  =>
              joined.connection
                .correlateBrowserTrace(
                  command,
                  traceReference(eventRefs._1),
                  traceReference(eventRefs._2)
                ) *>
                offer(command).onError(_ =>
                  joined.correlations.update(_ - command) *>
                    ZIO.succeed(joined.connection.cancelTrace(command))
                )
          }
    }

  private def traceReference(reference: PhoenixRef): Option[String] = reference match
    case PhoenixRef.Null         => None
    case PhoenixRef.Value(value) => Some(value)

  private def traceProtocol(envelope: PhoenixEnvelope): Json =
    Json.Obj(
      "joinReference"    -> traceReference(envelope.joinRef).fold[Json](Json.Null)(Json.Str(_)),
      "messageReference" -> traceReference(envelope.ref).fold[Json](Json.Null)(Json.Str(_)),
      "topic"            -> Json.Str(envelope.topic),
      "event"            -> Json.Str(envelope.event),
  ...
  private def lifecycleError(
    stage: LifecycleFailureStage,
    cause: Cause[Throwable]
  ): LifecycleError =
    if cause.isInterruptedOnly then LifecycleError(LifecycleFailure.Interrupted)
    else
      LifecycleError(
        LifecycleFailure.Stage(stage),
