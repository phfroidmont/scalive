...
| `ErrorLiveView` | Simulates child crashes, rejoins, reloads, and console messages by mutating parent state and synthetic log payloads. | Real parent and child LiveViews crash, rejoin, give up, link, remount, and drive client error classes without synthetic log injection. | Use real nested child LiveViews and actual failure/remount paths, then remove parent-side crash simulation. | Gap |
...
| `Issue3530LiveView` | Uses stream items with hook lifecycle logging, but the hook is no longer rendered inside the nested LiveView. | Streamed children are nested LiveViews with hook mount/destroy behavior under reset and insert operations. | Restore the hook inside the streamed nested LiveView and fix the runtime/client behavior needed for nested stream reset and insert lifecycle parity. | Gap |
| `Issue3612LiveView` | Renders normal navigate links directly in the page. | A sticky child LiveView remains connected while its own event handler issues `push_navigate` between pages. | Add a sticky child fixture whose click handlers perform server-side navigation. | Gap |
| `Issue3647LiveView` | Button click directly marks an upload complete and changes an unrelated input value. | Hook-driven `uploadTo` starts an auto-upload while another input event locks a different form, including ignored upload input data-attribute merging. | Implement the real upload fixture with `allow_upload`, external form-owned file input, JS hook upload, and concurrent input event. | Gap |
| `Issue3651LiveView` | Renders a static hidden notice and total `0`. | Nested hooks with a changing DOM id, pushed events, destroyed cleanup, and ignored notice prevent ghost elements and event explosions. | Add the outer/inner hooks, dynamic id change, pushed client event loop, counter, and ignored notice behavior. | Gap |
| `Issue3656LiveView` | A custom hook manually clears the click-loading class after dispatch. | `phx-click-loading` is added and removed by LiveView itself for a normal link inside a sticky LiveView during navigation. | Remove manual class clearing and use a sticky LiveView containing a normal navigate link. | Gap |
| `Issue3658LiveView` | Places the `phx-remove` element in the root view and uses a no-op click instead of parent navigation. | A `phx-remove` element inside a sticky LiveView is not removed when the parent navigates. | Render `#foo` inside a sticky child and trigger real navigation from the parent link. | Gap |
| `Issue3681LiveView` | Hard-codes the final stream DOM. | A sticky LiveView stream must not reset or collide when another LiveView initializes and resets a stream with the same name. | Implement real sticky and away LiveViews with their separate stream setup and reset behavior. | Gap |
...
| `Issue3686LiveView` | Seeds expected flash text by route constructor and navigates client-side. | Flash propagates through server `push_navigate`, server `redirect`, and another `push_navigate`. | Exercise real server flash storage/transport across navigation and redirect. | Gap |
| `Issue3709LiveView` | Provides one navigate link and a no-op button. | Many rapid patch navigations race with pending diffs while a LiveComponent id changes with the route id. | Add multiple patch links, a button that clicks them rapidly, and a component keyed by current route id. | Gap |
